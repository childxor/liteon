...
namespace IPS_TH.ViewComponents
{
    public class ModuleMenuViewComponent : ViewComponent
    {
        ...
        public ModuleMenuViewComponent(ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            try
            {
                ...
                var language = HttpContext.Session.GetString("Language") ?? "th";
                ...
                if (isAdmin)
                {
                    // กรณีเป็นผู้ดูแลระบบ ให้เห็นทุกโมดูลที่ active
                    await BuildAdminMenu(modules, ModuleMenu, language);
                }
                else
                {
                ...
                ModuleMenu = ModuleMenu.OrderBy(m => ((dynamic)m).Order).ToList();
                return View(ModuleMenu);
            }
            catch (Exception ex)
            {
                // บันทึกข้อผิดพลาด (ในสภาพแวดล้อมจริงควรใช้ logging framework)
                System.Diagnostics.Debug.WriteLine($"Error in ModuleMenuViewComponent: {ex.Message}");
                return View(new List<object>());
            }
        }

        // สร้างเมนูสำหรับผู้ดูแลระบบ
        private Task BuildAdminMenu(List<sys_module> modules, List<object> ModuleMenu, string language)
        {
            foreach (var module in modules.Where(m => m.ParentId == null))
            {
                ...
                ModuleMenu.Add(new {
                    ...
                    module.Id,
                    module.ParentId,
                    Children = moduleChild.Select(mc => new {
