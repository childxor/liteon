...
class Product_type extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['module'] = 'main/mas/product_type';
    ...
    function add()
    {
        ...
        if (!$this->db->insert('mas_product_type', $data)) {
            $this->session->set_flashdata('type', 'error');
            $this->session->set_flashdata('msg', $this->lang->sys_msg_cannot_add);
            /* Create Log Data  */
            $this->log_lib->write_log('error-product-type-001=> ' . $this->session->flashdata('msg'), json_encode($this->db->last_query()));
        } else {
            $this->session->set_flashdata('type', 'success');
            $this->session->set_flashdata('msg', $this->lang->sys_msg_save_success);
            $this->log_lib->write_log('Product_type => ' . $this->session->flashdata('msg'), json_encode($this->db->last_query()));
        }
    ...
    function edit()
    {
        ...
        $id = $this->efs_lib->decrypt_segment($this->input->post('id'));
        $this->db->where('id', $id);
        if (!$this->db->update('mas_product_type', $data)) {
            $this->session->set_flashdata('type', 'error');
            $this->session->set_flashdata('msg', $this->lang->sys_msg_cannot_update);
            /* Create Log Data  */
            $this->log_lib->write_log('error-product-type-002=> ' . $this->session->flashdata('msg'), json_encode($data));
        } else {
            $this->session->set_flashdata('type', 'success');
            $this->session->set_flashdata('msg', $this->lang->sys_msg_save_success);
            $this->log_lib->write_log('Product_type => ' . $this->session->flashdata('msg'), json_encode($this->db->last_query()));
    ...
    function delete()
    {
        ...
        if (!$this->db->update('mas_product_type', $data)) {
            ...
            /* Create Log Data  */
            $this->log_lib->write_log('error-product-type-003=> ' . $this->session->flashdata('msg'), json_encode($data));
        } else {
            $this->session->set_flashdata('type', 'success');
    ...
    function change_active()
    {
        ...
        if (!$this->db->update('mas_product_type', $data)) {
            ...
            $this->session->set_flashdata('msg', $this->lang->sys_msg_cannot_update);
            /* Create Log Data  */
            $this->log_lib->write_log('error-product-type-004=> ' . $this->session->flashdata('msg'), json_encode($data));
        } else {
            $this->session->set_flashdata('type', 'success');
            $this->session->set_flashdata('msg', $this->lang->sys_msg_save_success);
            $this->log_lib->write_log('Product_type => ' . $this->session->flashdata('msg'), json_encode($this->db->last_query()));
