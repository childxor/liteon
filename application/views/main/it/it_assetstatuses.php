    ...
    <div class="card">
        <div class="card-body">
            <form id="searchForm">
                ...
                <div class="row mb-4">
                    ...
                    <?php if ($this->efs_lib->is_can($permission, "add")) : ?>
                        <div class="col-md-4 col-lg-3 mb-3">
                            <button type="button" id="addBtn" class="btn btn-success btn-block" data-toggle="modal" data-target="#locationsModal">
            ...
            <div class="table-responsive">
                <table id="locationsTable" class="table table-bordered table-striped table-hover" style="width:100%"></table>
            </div>
        </div>
    </div>

    <?php if ($this->efs_lib->is_can($permission, "edit")) { ?>
        <!-- Asset Statuses Modal -->
        <div class="modal fade" id="locationsModal" tabindex="-1" role="dialog" aria-labelledby="locationsModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header bg-primary text-white">
                        <h5 class="modal-title" id="locationsModalLabel"><i class="fas fa-desktop mr-2"></i><span id="modalTitle">Add Asset Statuses</span></h5>
                    ...
                    <div class="modal-body">
                        <form id="locationsForm">
                            ...
                            <div class="form-group">
                                <label for="locationsName"><i class="fas fa-tag mr-2"></i>Name</label>
                                <input type="text" class="form-control" id="locationsName" name="name" required>
                            </div>
                            <div class="form-group">
                                <label for="locationsDescription"><i class="fas fa-info-circle mr-2"></i>Description</label>
                                <textarea class="form-control" id="locationsDescription" name="description" rows="3"></textarea>
                            </div>
    ...
    <script type="text/javascript">
        ...
        ! function(window, document, $) {
            ...
            locationsTable = $('#locationsTable').DataTable({
                ...
                "ajax": {
                    "url": "<?php echo base_url('it/it_assetstatuses/getlocations'); ?>",
                    "type": "POST",
                    "data": function(data) {
                        data.name = $('#name').val();
                ...
                "columns": [{
                    ...
                    <?php if ($this->efs_lib->is_can($permission, "edit")) { ?> {
                            ...
                            "width": "100px",
                            "render": function(data, type, row, meta) {
                                var editBtn = '<button class="editlocations btn btn-sm btn-warning mr-1" data-id="' + row.id + '"><i class="fas fa-edit"></i></button>';
                                var deleteBtn = '<button class="deletelocations btn btn-sm btn-danger" data-id="' + row.id + '"><i class="fas fa-trash"></i></button>';
        ...
        $(document).ready(function() {
            ...
            $('#is_active').change(function() {
                locationsTable.column(3).search($(this).val()).draw();
            });
            ...
            $('#locationsForm').submit(function(e) {
                ...
                $.ajax({
                    url: '<?php echo base_url('it/it_assetstatuses/savelocations'); ?>',
                    type: 'POST',
                    data: formData,
                    ...
                    error: function() {
                        swal.fire('Error', 'An error occurred while saving the Asset Statuses', 'error');
                    }
                });
            });

            $(document).on('click', '.editlocations', function() {
                var id = $(this).data('id');
                var row = locationsTable.row($(this).closest('tr')).data();
                $('#id').val(row.id);
                $('#locationsName').val(row.name);
                $('#locationsDescription').val(row.description);
                $('#modalTitle').text('Edit Asset Statuses');
            ...
            $(document).on('click', '.deletelocations', function() {
                ...
                swal.fire({
                    ...
                    if (result) {
                        $.ajax({
                            url: '<?php echo base_url('it/it_assetstatuses/deletelocations/'); ?>' + id,
                            type: 'POST',
                            success: function(response) {
                                locationsTable.ajax.reload();
                                swal.fire('Deleted!', 'Asset Statuses has been deleted.', 'success', {
                                    timer: 2000
                                });
                            }
                        });
            ...
            $(document).on('change', '.isActive', function() {
                ...
                $.ajax({
                    url: '<?php echo base_url('it/it_assetstatuses/updateIsActive'); ?>',
                    type: 'POST',
                    data: {
                        id: id,
            ...
            $('#addBtn').click(function() {
                $('#locationsForm')[0].reset();
                $('#id').val('');
                $('#modalTitle').text('Add Asset Statuses');
            });
        });
    </script>
<?php
