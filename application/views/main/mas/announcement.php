...
<?php if (!empty($this->efs_lib->is_can($permission, "view"))) { /* Can View */ ?>
                        ...
                        <table id="mTable" class="table table-bordered table-striped table-hover datatable" style="width:100%">
                            <thead>
                                <tr>
                                    ...
                                    <th><?php echo $lang_module->lbl_title; ?></th>
                                    <th><?php echo $lang_module->lbl_range_date; ?></th>
                                    <th class="no-sort text-center"><i class="fa fa-eye"></i></th>
    ...
    <?php if ($this->efs_lib->is_can($permission, "add")) { ?>
        <div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="addModalLabel">
            <form id="addForm" method="post" action="<?php echo base_url('mas/announcement/add'); ?>" novalidate>
                        ...
                        <div class="modal-body">
                            <div class='form-group'>
                                ...
                                <div class='input-group'>
                                    <input type='text' name="daterange" class="form-control datetime validated" autocomplete="off" required data-validation-required-message="<?php echo $lang_sys->msg_require; ?>" />
                            ...
                            <div class="form-group">
                                <label for="text" class="control-label"><?php echo $lang_module->lbl_title; ?><span class="text-danger">*</span></label>
                                <div class="controls">
                                    <input type="text" name="title" class="form-control validated" placeholder="" required data-validation-required-message="<?php echo $lang_sys->msg_require; ?>" autocomplete="off">
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="text" class="control-label"><?php echo $lang_module->lbl_detail; ?><span class="text-danger">*</span></label>
                                <div class="controls">
                                    <textarea name="txt" class="form-control validated summernote" rows="4" placeholder="" required data-validation-required-message="<?php echo $lang_sys->msg_require; ?>"></textarea>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="control-label"><?php echo $lang_module->lbl_must_accept; ?></label>
                                <div class="controls">
                                    <select class="form-control" name="must_accept">
                                        <option value="0"><?php echo $lang_module->lbl_not_accept; ?></option>
                                        <option value="1"><?php echo $lang_module->lbl_accept; ?></option>
                                    </select>
                                </div>
                            </div>
                        </div>
    ...
    <?php if ($this->efs_lib->is_can($permission, "edit")) { ?>
        <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel">
            <form id="editForm" method="post" action="<?php echo base_url('mas/announcement/edit'); ?>" novalidate>
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label for="text" class="control-label"><?php echo $lang_module->lbl_detail; ?><span class="text-danger">*</span></label>
                                <div class="controls">
                                    <textarea name="txt" class="form-control validated summernote" rows="4" placeholder="" required></textarea>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="control-label"><?php echo $lang_module->lbl_must_accept; ?></label>
    ...
    <script type="text/javascript">
        ...
        ! function(window, document, $) {
            ...
            provTable = $('#mTable').DataTable({
                ...
                "ajax": "<?php echo base_url('mas/announcement/ajax_list'); ?>",
                "deferRender": true,
                "aLengthMenu": [
                    [10, 25, 50, 100],
                    [10, 25, 50, 100]
                    ...
                    <?php if ($this->efs_lib->is_can($permission, "delete")) { ?> {
                            ...
                            render: function(data, type) {
                                if (data.created_username == 'system') {
                                    ...
                                    return '<button type="button" class="btn btn-danger btn-xs confirmDelete" data-href="<?php echo base_url('mas/announcement/delete/'); ?>' + data.id + '"><i class="fas fa-trash-alt"></i></button>';
                                }
                            }
                        },
                    <?php } ?>
                ...
                "fnDrawCallback": function() {
                    ...
                    $('.switch[type="checkbox"]').on('switchChange.bootstrapSwitch', function(event, state) {
                        $(location).attr('href', '<?php echo base_url() ?>mas/announcement/change_active/' + $(this).data('id') + '.' + $(this).data('active'));
                    });
                },
            });
            provTable.on('row-reorder', function(e, diff, edit) {
                for (var i = 0, ien = diff.length; i < ien; i++) {
                    ...
                    $.post('<?php echo base_url('mas/announcement/sort'); ?>', {
        ...
        $(document).ready(function() {
            ...
            <?php if ($this->efs_lib->is_can($permission, "edit")) { ?>
                $('#mTable tbody').on('click', 'tr td #editBtn', function() {
                    ...
                    $form.find('.form-error').remove();
                    $form.find('.help-block').html('');
                    $.post("<?php echo base_url('mas/announcement/ajax_data'); ?>", {
                            id: $(this).data('id')
                        },
                        function(data) {
                            $form.find('[name="id"]').val(data['id']);
                            $form.find('[name="title"]').val(data['title']);
                            $form.find('[name="must_accept"]').val(data['must_accept']);
                            $form.find('[name="txt"]').summernote("code", data['txt']);
                            $form.find('[name="daterange"]').val(data['show'].replace(':00.000', '') + ' - ' + data['hide'].replace(':00.000', '')).daterangepicker({
                                timePicker: true,
