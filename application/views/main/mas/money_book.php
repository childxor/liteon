...
<?php if (!empty($this->efs_lib->is_can($permission, "view"))) { /* Can View */ ?>
                        ...
                        <table id="mTable" class="table table-bordered table-striped table-hover datatable" style="width:100%">
                            <thead>
                                <tr>
                                    ...
                                    <th><?php echo $lang_module->lbl_material; ?></th>
                                    <th><?php echo $lang_module->lbl_price; ?></th>
                                    <th><?php echo $lang_module->lbl_date; ?></th>
    ...
    <?php if ($this->efs_lib->is_can($permission, "add")) { ?>
        ...
        <div class="modal fade" id="addModal" tabindex="-1" role="dialog">
            <form id="addForm" method="post" action="<?php echo base_url('mas/supplier/money_book_add'); ?>" novalidate>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                <label class="control-label"><?php echo $lang_module->lbl_material; ?><span class="text-danger">*</span></label>
                                <div class="controls">
                            ...
                            <div class="form-group">
                                <label class="control-label"><?php echo $lang_module->lbl_price; ?><span class="text-danger">*</span></label>
                                <div class="controls">
                                    <input type="number" class="form-control validated" step="0.01" name="price" required data-validation-required-message="<?php echo $lang_sys->msg_require; ?>">
                                    <!-- <input type="number" class="form-control validated" name="price" step="0.01" required data-validation-required-message="<?php echo $lang_sys->msg_require; ?>"> -->
                                </div>
                            </div>
                            <div class="form-group">
                                <label>
                                    <input type="checkbox" name="use_qty" checked="checked">
                                    <?php echo $lang_module->lbl_use_qty; ?>
                                </label>
                            </div>
                            <div class="form-group">
                                <label class="control-label"><?php echo $lang_module->lbl_description; ?></label>
                                <div class="controls">
                                    <textarea name="remark" class="form-control" rows="3" placeholder=""></textarea>
                                    <small class="text-danger">คำอธิบาย => ใส่เครื่องหมาย *# ตามด้วยข้อความเพื่อให้แสดงในหน้าใบขอเสนอซื้อ(PR)</small>
                                </div>
                            </div>
                        </div>
    ...
    <?php if ($this->efs_lib->is_can($permission, "edit")) { ?>
        <div class="modal fade" id="editModal" tabindex="-1" role="dialog">
            <form id="editForm" method="post" action="<?php echo base_url('mas/supplier/money_book_edit'); ?>" novalidate>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label>
                                    <input type="checkbox" name="use_qty">
                                    <?php echo $lang_module->lbl_use_qty; ?>
                                </label>
                            </div>
                            <div class="form-group">
    ...
    <script type="text/javascript">
        ...
        ! function(window, document, $) {
            ...
            provTable = $('#mTable').DataTable({
                ...
                "ajax": "<?php echo base_url('mas/supplier/money_book_ajax_list/' . $this->uri->segment(4)); ?>",
                "deferRender": true,
                "aLengthMenu": [
                    [10, 25, 50, 100],
                    [10, 25, 50, 100]
                    ...
                    <?php if ($this->efs_lib->is_can($permission, "delete")) { ?> {
                            ...
                            render: function(data, type) {
                                if (data.created_username == 'system') {
                                    ...
                                    return '<button type="button" class="btn btn-danger btn-xs confirmDelete" data-href="<?php echo base_url('mas/supplier/money_book_delete/'); ?>' + data.id + '"><i class="fas fa-trash-alt"></i></button>';
                                }
                            }
                        },
                    <?php } ?>
                ...
                "fnDrawCallback": function() {
                    ...
                    $('.switch[type="checkbox"]').on('switchChange.bootstrapSwitch', function(event, state) {
                        $(location).attr('href', '<?php echo base_url() ?>mas/supplier/money_book_change_active/' + $(this).data('id') + '.' + $(this).data('active'));
                    });
                },
            });
        ...
        $(document).ready(function() {
            ...
            <?php if ($this->efs_lib->is_can($permission, "edit")) { ?>
                $('#mTable tbody').on('click', 'tr td #editBtn', function() {
                    ...
                    $form.find('.form-error').remove();
                    $form.find('.help-block').html('');
                    $.post("<?php echo base_url('mas/supplier/money_book_ajax_data'); ?>", {
                            id: $(this).data('id')
                        },
                        function(data) {
                            ...
                            $form.find('[name="use_qty"]').prop('checked', Boolean(Number(data['use_qty'])));
                        }, "json");
                });
            <?php } ?>
