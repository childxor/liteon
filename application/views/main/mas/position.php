...
<?php if (!empty($this->efs_lib->is_can($permission, "view"))) { /* Can View */ ?>
    ...
    <?php if ($this->efs_lib->is_can($permission, "add")) { ?>
        <div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="addModalLabel">
            <form id="addForm" method="post" action="<?php echo base_url('mas/position/add'); ?>" novalidate>
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label class="control-label"><?php echo $lang_module->lbl_short_position; ?><span class="text-danger">*</span></label>
                                <div class="controls">
                                    <input type="text" class="form-control validated" name="short_name" required data-validation-required-message="<?php echo $lang_sys->msg_require; ?>" autocomplete="off">
                                </div>
                            </div>
                            <div class="form-group">
    ...
    <?php if ($this->efs_lib->is_can($permission, "edit")) { ?>
        <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel">
            <form id="editForm" method="post" action="<?php echo base_url('mas/position/edit'); ?>" novalidate>
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
    ...
    <script type="text/javascript">
        ...
        ! function(window, document, $) {
            ...
            provTable = $('#mTable').DataTable({
                ...
                "ajax": "<?php echo base_url('mas/position/ajax_list'); ?>",
                "deferRender": true,
                "aLengthMenu": [
                    [10, 25, 50, 100],
                    [10, 25, 50, 100]
                ...
                "columns": [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            return data.name + ' (' + data.short_name + ')';
                        }
                    },
                    {
                        "data": "description"
                    ...
                    <?php if ($this->efs_lib->is_can($permission, "delete")) { ?> {
                            ...
                            render: function(data, type) {
                                if (data.created_username == 'system') {
                                    ...
                                    return '<button type="button" class="btn btn-danger btn-xs confirmDelete" data-href="<?php echo base_url('mas/position/delete/'); ?>' + data.id + '"><i class="fas fa-trash-alt"></i></button>';
                                }
                            }
                        },
                    <?php } ?>
                ...
                "fnDrawCallback": function() {
                    ...
                    $('.switch[type="checkbox"]').on('switchChange.bootstrapSwitch', function(event, state) {
                        $(location).attr('href', '<?php echo base_url() ?>mas/position/change_active/' + $(this).data('id') + '.' + $(this).data('active'));
                    });
                },
            });
            provTable.on('row-reorder', function(e, diff, edit) {
                for (var i = 0, ien = diff.length; i < ien; i++) {
                    ...
                    $.post('<?php echo base_url('mas/position/sort'); ?>', {
        ...
        $(document).ready(function() {
            ...
            <?php if ($this->efs_lib->is_can($permission, "edit")) { ?>
                $('#mTable tbody').on('click', 'tr td #editBtn', function() {
                    ...
                    $form.find('.form-error').remove();
                    $form.find('.help-block').html('');
                    $.post("<?php echo base_url('mas/position/ajax_data'); ?>", {
                            id: $(this).data('id')
                        },
                        function(data) {
                            $form.find('[name="id"]').val(data['id']);
                            $form.find('[name="name"]').val(data['name']);
                            $form.find('[name="short_name"]').val(data['short_name']);
