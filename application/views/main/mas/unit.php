...
<?php if (!empty($this->efs_lib->is_can($permission, "view"))) { /* Can View */ ?>
                        ...
                        <table id="mTable" class="table table-bordered table-striped table-hover datatable" style="width:100%">
                            <thead>
                                <tr>
                                    ...
                                    <th><?php echo $lang_module->lbl_unit; ?></th>
                                    <th class="is-small"><?php echo $lang_module->lbl_qty; ?></th>
    ...
    <?php if ($this->efs_lib->is_can($permission, "add")) { ?>
        <div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="addModalLabel">
            <form id="addForm" method="post" action="<?php echo base_url('mas/unit/add'); ?>" novalidate>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                <label class="control-label"><?php echo $lang_module->lbl_thai; ?><span class="text-danger">*</span></label>
                                <div class="controls">
                            ...
                            <div class="form-group">
                                <label class="control-label"><?php echo $lang_module->lbl_english; ?></label>
                                <div class="controls">
                                    <input type="text" class="form-control validated" name="name_en" autocomplete="off">
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="rate" class="control-label"><?php echo $lang_module->lbl_qty; ?>/<?php echo $lang_module->lbl_unit; ?></label>
                                <div class="controls">
                                    <input type="number" class="form-control validated" name="rate" required data-validation-required-message="<?php echo $lang_sys->msg_require; ?>" autocomplete="off">
                                </div>
                            </div>
                        </div>
    ...
    <?php if ($this->efs_lib->is_can($permission, "edit")) { ?>
        <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel">
            <form id="editForm" method="post" action="<?php echo base_url('mas/unit/edit'); ?>" novalidate>
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
    ...
    <script type="text/javascript">
        ...
        ! function(window, document, $) {
            ...
            provTable = $('#mTable').DataTable({
                ...
                "ajax": "<?php echo base_url('mas/unit/ajax_list'); ?>",
                "deferRender": true,
                "aLengthMenu": [
                    [10, 25, 50, 100],
                    [10, 25, 50, 100]
                ...
                "columns": [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type) {
                            return data.name_th + (data.name_en !== null ? ' / ' + data.name_en : '');
                        }
                    },
                    {
                        "data": "rate"
                    ...
                    <?php if ($this->efs_lib->is_can($permission, "delete")) { ?> {
                            ...
                            render: function(data, type) {
                                if (data.created_username == 'system') {
                                    ...
                                    return '<button type="button" class="btn btn-danger btn-xs confirmDelete" data-href="<?php echo base_url('mas/unit/delete/'); ?>' + data.id + '"><i class="fas fa-trash-alt"></i></button>';
                                }
                            }
                        },
                    <?php } ?>
                ...
                "fnDrawCallback": function() {
                    ...
                    $('.switch[type="checkbox"]').on('switchChange.bootstrapSwitch', function(event, state) {
                        $(location).attr('href', '<?php echo base_url() ?>mas/unit/change_active/' + $(this).data('id') + '.' + $(this).data('active'));
                    });
                },
            });
        ...
        $(document).ready(function() {
            ...
            <?php if ($this->efs_lib->is_can($permission, "edit")) { ?>
                $('#mTable tbody').on('click', 'tr td #editBtn', function() {
                    ...
                    $form.find('.form-error').remove();
                    $form.find('.help-block').html('');
                    $.post("<?php echo base_url('mas/unit/ajax_data'); ?>", {
                            id: $(this).data('id')
                        },
                        function(data) {
