...
<?php if (!empty($this->efs_lib->is_can($permission, "view"))) { /* Can View */ ?>
    ...
    <div class="row">
        <div class="col-12">
            <form id="addForm" method="post" action="<?php echo base_url('sys/role/add'); ?>" novalidate>
                <div class="row">
                    <div class="col-md-6">
                            ...
                            <div class="row">
                                <div class="col-sm-12 col-xs-12">
                                    <div class="form-group">
                                        ...
                                        <div class="controls">
                                            <input type="text" class="form-control validated" id="name" name="name" required data-validation-required-message="<?php echo $lang_sys->msg_require; ?>" data-validation-ajax-ajax="<?php echo base_url('sys/role/ajax_exist'); ?>" pattern="^[a-zA-Z][a-zA-Z0-9-_]{1,20}$" data-validation-pattern-message="<?php echo $lang_module->msg_ajax; ?>" autocomplete="off">
                                        </div>
                                    </div>
                                    <div class="form-group">
                                                    ...
                                                    ?>
                                                        <optgroup label="<i class='ti <?php echo $parent->icon; ?>'></i> <?php echo $parent->{"name_" . $this->session->userdata("user_profile")->cng_lang}; ?>">
                                                            <?php foreach ($subs as $sub) { ?>
                                                                <option id="opt_<?php echo $sub->id; ?>" value="<?php echo $sub->id; ?>"><?php echo $sub->{"name_" . $this->session->userdata("user_profile")->cng_lang}; ?></option>
                                                            <?php } ?>
                                                        <?php } ?>
                                                        </optgroup>
                                                    <?php } ?>
                                    ...
                                    <?php if (!empty($this->efs_lib->is_can($permission, "add"))) { // Add View 
                                    ?>
                                        <div class="form-group">
                                                ...
                                                <?php foreach ($subs as $sub) { ?>
                                                    <li class="">
                                                        <label class="custom-control custom-checkbox">
                                                            <input class="custom-control-input" type="checkbox" name="module_ids[]" value="<?php echo $sub->id; ?>">
