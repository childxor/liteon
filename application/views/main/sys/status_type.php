...
<?php if (!empty($this->efs_lib->is_can($permission, "view"))) { /* Can View */ ?>
                        ...
                        <table id="mTable" class="table table-bordered table-striped table-hover datatable" style="width:100%">
                            <thead>
                                <tr>
                                    ...
                                    <th><?php echo $lang_module->lbl_alias; ?></th>
                                    <th><?php echo $lang_module->lbl_name; ?></th>
                                    <th class="is-small text-center"><i class="fa fa-bars"></i></th>
    ...
    <?php if ($this->efs_lib->is_can($permission, "add")) { ?>
        <div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="addModalLabel">
            <form id="addForm" method="post" action="<?php echo base_url('sys/status_type/add'); ?>" novalidate>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                <label class="control-label"><?php echo $lang_module->lbl_alias; ?><span class="text-danger">*</span></label>
                                <div class="controls">
                                    <input type="text" class="form-control validated" name="alias" required data-validation-required-message="<?php echo $lang_sys->msg_require; ?>" autocomplete="off">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="control-label"><?php echo $lang_module->lbl_name; ?><span class="text-danger">*</span></label>
                                <div class="controls">
    ...
    <?php if ($this->efs_lib->is_can($permission, "edit")) { ?>
        <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel">
            <form id="editForm" method="post" action="<?php echo base_url('sys/status_type/edit'); ?>" novalidate>
    ...
    <script type="text/javascript">
        ...
        ! function(window, document, $) {
            ...
            provTable = $('#mTable').DataTable({
                ...
                "ajax": "<?php echo base_url('sys/status_type/ajax_list'); ?>",
                "deferRender": true,
                "aLengthMenu": [
                    [10, 25, 50, 100],
                    [10, 25, 50, 100]
                ...
                "columns": [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type) {
                            return '<a href="<?php echo base_url('sys/status_type/status/'); ?>' + data.id + '" class="btn btn-info btn-xs"><i class="fas fa-align-justify"></i></a>';
                        }
                    },
                    <?php if ($this->efs_lib->is_can($permission, "edit")) { ?> {
                            "data": {
                    ...
                    <?php if ($this->efs_lib->is_can($permission, "delete")) { ?> {
                            ...
                            render: function(data, type) {
                                if (data.created_username == 'system') {
                                    ...
                                    return '<button type="button" class="btn btn-danger btn-xs confirmDelete" data-href="<?php echo base_url('sys/status_type/delete/'); ?>' + data.id + '"><i class="fas fa-trash-alt"></i></button>';
                                }
                            }
                        },
                    <?php } ?>
                ...
                "fnDrawCallback": function() {
                    ...
                    $('.switch[type="checkbox"]').on('switchChange.bootstrapSwitch', function(event, state) {
                        $(location).attr('href', '<?php echo base_url() ?>sys/status_type/change_active/' + $(this).data('id') + '.' + $(this).data('active'));
                    });
                },
            });
            provTable.on('row-reorder', function(e, diff, edit) {
                for (var i = 0, ien = diff.length; i < ien; i++) {
                    ...
                    $.post('<?php echo base_url('sys/status_type/sort'); ?>', {
        ...
        $(document).ready(function() {
            ...
            <?php if ($this->efs_lib->is_can($permission, "edit")) { ?>
                $('#mTable tbody').on('click', 'tr td #editBtn', function() {
                    ...
                    $form.find('.form-error').remove();
                    $form.find('.help-block').html('');
                    $.post("<?php echo base_url('sys/status_type/ajax_data'); ?>", {
                            id: $(this).data('id')
                        },
                        function(data) {
