...
;(function (document, $) {
	...
	$.extend( SlideShow.prototype, {
		...
		set : function( force ) {
			var self = this;
console.info('== set; force: ' + force);
			// Check if reached last element
			if ( self.instance && self.instance.current && (force === true || self.instance.current.opts.loop || self.instance.currIndex < self.instance.group.length - 1 )) {
console.info('!!');
				self.timer = setTimeout(function() {

		console.info('>> ' + (self.instance.current.index + 1 % self.instance.group.length-1) );
					self.instance.jumpTo( self.instance.current.index  % self.instance.group.length-1 );
...
;(function (document, $) {
		...
		btnTpl : {
			share :
				'<button data-fancybox-share class="fancybox-button fancybox-button--share" title="{{SHARE}}">' +
					'<svg viewBox="0 0 40 40">' +
						'<path d="M6,30 C8,18 27,14 23,16 L23,10 L33,20 L23,29 L23,24 C14,24 7,28 6,30 Z">' +
					'</svg>' +
				'</button>'
		},
		shareTpl :
	...
	$(document).on('click', '[data-fancybox-share]', function() {
		...
		if ( f ) {
			$.fancybox.open({
				src  : f.translate( f, f.current.opts.shareTpl.replace( /\{\{src\}\}/g, encodeURI( f.current.src ) ).replace( /\{\{descr\}\}/g, encodeURI( f.current.caption || '' ) ) ),
				type : 'html',
				opts : {
					autoFocus : false,
...
//adding custom item to fancybox menu to rotate image
    $(document).on('onInit.fb', function (e, instance) {
        if ($('.fancybox-toolbar').find('#rotate_button').length === 0) {
            $('.fancybox-toolbar').prepend('<button id="rotate_button" class="fancybox-button" title="Rotate Image">'+
			'<svg viewBox="0 0 40 40">'+
				'<path d="M6,30 C8,18 27,14 23,16 L23,10 L33,20 L23,29 L23,24 C14,24 7,28 6,30 Z">'+
			'</svg>'+
			'</button>');
