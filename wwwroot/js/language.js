...
function getModuleIdFromUrl() {
    ...
    var lastSegment = segments.length > 0 ? segments[segments.length - 1] : 'Index';
 ...
 */
function loadLanguageData(moduleId, callback) {
    ...
    if (!moduleId) {
        ...
        if (!moduleId || moduleId === 'MQ==') {
            moduleId = $('body').data('module-id') || $('select#languageSelector').data('module-id');
        }
    ...
    var showLoader = function () {
        if ($('.page-loader').length) {
            ...
            $('body').append('<div class="temp-loader" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.3);z-index:9999;display:flex;justify-content:center;align-items:center;"><div style="color:white;background:#333;padding:15px;border-radius:5px;">กำลังโหลดข้อมูลภาษา...</div></div>');
        }
    };
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response && response.data) {
                ...
                try {
                    localStorage.setItem('translations_' + moduleId, JSON.stringify(response.data));
                ...
                console.error('ไม่สามารถโหลดข้อมูลภาษาได้:', response ? response.message : 'ไม่มีข้อมูลตอบกลับ');
                ...
                if (cachedData) {
                    ...
                    // เรียกใช้ callback ถ้ามี
                    if (typeof callback === 'function') {
                        callback(true, { data: cachedData.data, language: cachedData.language });
                    }
                } else {
...
function changeLanguage(language) {
    ...
    if ($('.page-loader').length) {
        ...
        // สร้าง loading indicator ชั่วคราวถ้าไม่มี
        $('body').append('<div class="temp-loader" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.3);z-index:9999;display:flex;justify-content:center;align-items:center;"><div style="color:white;background:#333;padding:15px;border-radius:5px;">กำลังเปลี่ยนภาษา...</div></div>');
    }
...
function translate(keyword) {
    ...
    if (!window.translations || Object.keys(window.translations).length === 0) {
        ...
        try {
            var moduleId = $('body').data('module-id') || $('select#languageSelector').data('module-id') || 'MQ==';
            var storedTranslations = localStorage.getItem('translations_' + moduleId);
...
function translatePage() {
    ...
    if (!window.translations || Object.keys(window.translations).length === 0) {
        ...
        try {
            ...
            // ถ้าไม่มีค่าจาก URL ให้ลองดึงจาก data attribute
            if (!moduleId || moduleId === 'MQ==') {
                moduleId = $('body').data('module-id') || $('select#languageSelector').data('module-id') || 'MQ==';
            }
    ...
    $('button[data-translate-value], input[type="button"][data-translate-value], input[type="submit"][data-translate-value]').each(function () {
        ...
        columns: [
            ...
            { data: 'zh' },
            {
                ...
                render: function (data) {
                    return data ? '<span class="badge bg-success">ใช้งาน</span>' :
